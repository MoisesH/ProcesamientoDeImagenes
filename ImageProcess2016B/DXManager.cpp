...
bool CDXManager::Initialize(HWND hWnd, bool bUseWARP, IDXGIAdapter* pAdapter)
{
	...
	dscd.Windowed = true;///////////////////////////////////////////////////////// al liberar la App cambiar a False
	...
	if (bUseWARP)
	{
		D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_WARP, NULL, 0, &Requested, 1, 
			D3D11_SDK_VERSION, &dscd, &m_pSwapChain, &m_pDevice, &Detected, &m_pContext);
	}
	else
	{
		if (pAdapter)
		{
			HRESULT hr = D3D11CreateDeviceAndSwapChain(pAdapter, D3D_DRIVER_TYPE_UNKNOWN, 0, 0, &Requested, 1, 
				D3D11_SDK_VERSION, &dscd, &m_pSwapChain, &m_pDevice, &Detected, &m_pContext);
		...
		{
			HRESULT hr = D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_UNKNOWN, 0, 0, &Requested, 1,
				D3D11_SDK_VERSION, &dscd, &m_pSwapChain, &m_pDevice, &Detected, &m_pContext);
			if (FAILED(hr)) return false;
		}
	}
